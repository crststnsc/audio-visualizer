...
pub fn main() anyerror!void {
    const backendsArray = [_]c_uint{ @intCast(ma.ma_backend_wasapi) };
    const backends: [*c]const c_uint = &backendsArray;
    var device: ma.ma_device = std.mem.zeroes(ma.ma_device);

    const allocator = std.heap.page_allocator;
    const userData = try allocator.alloc(Complex, 512);

    var deviceConfig = ma.ma_device_config_init(ma.ma_device_type_loopback);
    ...
    var result = ma.ma_device_init_ex(backends, @sizeOf(@TypeOf(backends))/@sizeOf(@TypeOf(backends[0])), null, &deviceConfig, &device);
    if (result != ma.MA_SUCCESS) {
        std.debug.print("Failed to initialize loopback device", .{});
